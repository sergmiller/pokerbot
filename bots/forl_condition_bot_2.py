...
class ConditionPlayer(BasePokerPlayer):
    def declare_action(self, valid_actions, hole_card, round_state):
        ...
        free_flop = self.seat == round_state["big_blind_pos"] and call_amount == 30

        if decision_function > 100:
            action = raise_action_info["action"]
            amount = min(pot_size, 4 * raise_amount_min)
            if raise_amount_min == -1:
                action, amount = call_action_info["action"], call_amount
